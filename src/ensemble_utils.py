...
class ChronologicalStackingEnsembleMultiClass:
    def __init__(self, estimators, final_estimator, cv, n_classes=3):
        self.estimators = estimators
        self.final_estimator = final_estimator
        self.cv = cv
    ...
    def fit(self, X, y):
        ...
        for train_idx, val_idx in self.cv.split(X_frame):
            ...
            for _, estimator in self.estimators:
                fitted = clone(estimator)
                fitted.fit(X_train_fold, y_train_fold)
                proba = fitted.predict_proba(X_val_fold)
                fold_predictions.append(proba)

            oof_parts.append(np.hstack(fold_predictions))
            target_parts.append(y_array[val_idx])
