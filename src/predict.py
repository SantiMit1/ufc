...
def predict_event_valuebets(event_fights: list[dict], bankroll: float) -> None:
    with open(FIGHTS_PATH) as f:
        fights = json.load(f)
    ...
    model_type = feature_meta.get("model_type", "stacking")
    if model_type in ("lightgbm", "stacking"):
        lgb_base = model.named_estimators_.get("lgbm") if model_type == "stacking" else model
        if lgb_base is not None and hasattr(lgb_base, "named_steps"):
            lgb_base = lgb_base.named_steps.get("logreg", lgb_base)
        if lgb_base is not None:
            shap_explainer = shap.TreeExplainer(lgb_base)
    ...
    for i, fight in enumerate(event_fights, 1):
        f1 = fight["fighter_a"]
        f2 = fight["fighter_b"]
        ...
        for fighter, prob, odds_v, market_p, edge_v in [
